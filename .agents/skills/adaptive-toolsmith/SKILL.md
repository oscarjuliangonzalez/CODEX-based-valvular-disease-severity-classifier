...
Create versioned tools with manifests, tests, limitations, and registry updates after validation. This skill supports research and decision-support development only; it must not be used to claim autonomous clinical diagnosis.

## Trigger Conditions

Use this skill when the active Codex task matches the description above or when an AgentTask lists adaptive-toolsmith in required_skills. Do not use it outside AR-focused echocardiography research workflows.
...
8. Populate schemas/agent_result.schema.json for result.json and validate new tool manifests against schemas/tool_manifest.schema.json.
...
Return valid JSON matching schemas/agent_result.schema.json for result.json; tool manifests must also match schemas/tool_manifest.schema.json. Prose-only output is invalid.
...
```json
{
  ...
  "skills_used": ["adaptive-toolsmith"],
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "output_artifacts": [],
  "measurements": [],
  "evidence": {},
