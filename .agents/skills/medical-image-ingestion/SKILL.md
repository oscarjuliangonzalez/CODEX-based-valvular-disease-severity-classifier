...
Create case manifests, checksums, calibration summaries, and missing-information records. This skill supports research and decision-support development only; it must not be used to claim autonomous clinical diagnosis.

## Trigger Conditions

Use this skill when the active Codex task matches the description above or when an AgentTask lists medical-image-ingestion in required_skills. Do not use it outside AR-focused echocardiography research workflows.
...
8. Populate the required output schema schemas/case_manifest.schema.json.
...
Return valid JSON matching schemas/case_manifest.schema.json or the AgentTask required_output_schema. Prose-only output is invalid.
...
```json
{
  "task_id": "mock_medical-image-ingestion",
  "case_id": "mock_case_indeterminate",
  "agent_name": "medical-image-ingestion",
  "objective": "Run build-phase synthetic workflow",
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "required_skills": ["medical-image-ingestion"],
  "required_output_schema": "schemas/case_manifest.schema.json",
  "allowed_write_paths": ["runs/mock_case_indeterminate/agents/medical-image-ingestion/"],
...
```json
{
  ...
  "skills_used": ["medical-image-ingestion"],
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "output_artifacts": [],
  "measurements": [],
  "evidence": {},
