...
Create inspectable masks in original coordinates with uncertainty and quality flags. This skill supports research and decision-support development only; it must not be used to claim autonomous clinical diagnosis.

## Trigger Conditions

Use this skill when the active Codex task matches the description above or when an AgentTask lists ar-mask-generation in required_skills. Do not use it outside AR-focused echocardiography research workflows.
...
8. Populate schemas/agent_result.schema.json for result.json and validate individual mask artifacts against schemas/mask_artifact.schema.json.
...
Return valid JSON matching schemas/agent_result.schema.json for result.json; artifact objects must also match their artifact schemas. Prose-only output is invalid.
...
```json
{
  ...
  "skills_used": ["ar-mask-generation"],
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "output_artifacts": [],
  "measurements": [],
  "evidence": {},
