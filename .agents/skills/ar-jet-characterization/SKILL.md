...
Describe jet morphology and downstream measurement validity while preserving limitations. This skill supports research and decision-support development only; it must not be used to claim autonomous clinical diagnosis.

## Trigger Conditions

Use this skill when the active Codex task matches the description above or when an AgentTask lists ar-jet-characterization in required_skills. Do not use it outside AR-focused echocardiography research workflows.
...
```json
{
  "task_id": "mock_ar-jet-characterization",
  "case_id": "mock_case_indeterminate",
  "agent_name": "ar-jet-characterization",
  "objective": "Run build-phase synthetic workflow",
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "required_skills": ["ar-jet-characterization"],
  "required_output_schema": "schemas/agent_result.schema.json",
  "allowed_write_paths": ["runs/mock_case_indeterminate/agents/ar-jet-characterization/"],
...
```json
{
  ...
  "skills_used": ["ar-jet-characterization"],
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "output_artifacts": [],
  "measurements": [],
  "evidence": {},
