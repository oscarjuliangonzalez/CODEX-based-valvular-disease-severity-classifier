...
Coordinate all specialist agents, preserve independent JSON results, and stop at indeterminate when evidence is insufficient. This skill supports research and decision-support development only; it must not be used to claim autonomous clinical diagnosis.

## Trigger Conditions

Use this skill when the active Codex task matches the description above or when an AgentTask lists ar-case-orchestration in required_skills. Do not use it outside AR-focused echocardiography research workflows.
...
8. Populate schemas/agent_result.schema.json for the report-generator result.json and validate the final case report against schemas/final_report.schema.json.
...
Return valid JSON matching schemas/agent_result.schema.json for result.json; final reports must also match schemas/final_report.schema.json. Prose-only output is invalid.
