...
Apply deterministic volumetric formulas with uncertainty and invalid-assumption detection. This skill supports research and decision-support development only; it must not be used to claim autonomous clinical diagnosis.

## Trigger Conditions

Use this skill when the active Codex task matches the description above or when an AgentTask lists ar-doppler-volumetrics in required_skills. Do not use it outside AR-focused echocardiography research workflows.
...
```json
{
  "task_id": "mock_ar-doppler-volumetrics",
  "case_id": "mock_case_indeterminate",
  "agent_name": "ar-doppler-volumetrics",
  "objective": "Run build-phase synthetic workflow",
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "required_skills": ["ar-doppler-volumetrics"],
  "required_output_schema": "schemas/agent_result.schema.json",
  "allowed_write_paths": ["runs/mock_case_indeterminate/agents/ar-doppler-volumetrics/"],
...
```json
{
  ...
  "skills_used": ["ar-doppler-volumetrics"],
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "output_artifacts": [],
  "measurements": [],
  "evidence": {},
