...
Use ECG when available, otherwise validated LV area/volume curves and cycle checks. This skill supports research and decision-support development only; it must not be used to claim autonomous clinical diagnosis.

## Trigger Conditions

Use this skill when the active Codex task matches the description above or when an AgentTask lists cardiac-phase-detection in required_skills. Do not use it outside AR-focused echocardiography research workflows.
...
```json
{
  "task_id": "mock_cardiac-phase-detection",
  "case_id": "mock_case_indeterminate",
  "agent_name": "cardiac-phase-detection",
  "objective": "Run build-phase synthetic workflow",
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "required_skills": ["cardiac-phase-detection"],
  "required_output_schema": "schemas/agent_result.schema.json",
  "allowed_write_paths": ["runs/mock_case_indeterminate/agents/cardiac-phase-detection/"],
...
```json
{
  ...
  "skills_used": ["cardiac-phase-detection"],
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "output_artifacts": [],
  "measurements": [],
  "evidence": {},
