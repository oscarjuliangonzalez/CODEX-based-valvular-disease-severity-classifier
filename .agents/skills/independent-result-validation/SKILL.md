...
Accept, warn, reject, or request revision with structured reasons and correction tasks. This skill supports research and decision-support development only; it must not be used to claim autonomous clinical diagnosis.

## Trigger Conditions

Use this skill when the active Codex task matches the description above or when an AgentTask lists independent-result-validation in required_skills. Do not use it outside AR-focused echocardiography research workflows.
...
8. Populate the required output schema schemas/agent_result.schema.json.
...
Return valid JSON matching schemas/agent_result.schema.json or the AgentTask required_output_schema. Prose-only output is invalid.
...
```json
{
  "task_id": "mock_independent-result-validation",
  "case_id": "mock_case_indeterminate",
  "agent_name": "independent-result-validation",
  "objective": "Run build-phase synthetic workflow",
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "required_skills": ["independent-result-validation"],
  "required_output_schema": "schemas/agent_result.schema.json",
  "allowed_write_paths": ["runs/mock_case_indeterminate/agents/independent-result-validation/"],
...
```json
{
  ...
  "skills_used": ["independent-result-validation"],
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "output_artifacts": [],
  "measurements": [],
  "evidence": {},
