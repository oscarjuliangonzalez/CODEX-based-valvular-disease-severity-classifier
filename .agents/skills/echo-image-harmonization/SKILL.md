...
Preserve original coordinates, transforms, and calibration while generating harmonized artifacts. This skill supports research and decision-support development only; it must not be used to claim autonomous clinical diagnosis.

## Trigger Conditions

Use this skill when the active Codex task matches the description above or when an AgentTask lists echo-image-harmonization in required_skills. Do not use it outside AR-focused echocardiography research workflows.
...
```json
{
  "task_id": "mock_echo-image-harmonization",
  "case_id": "mock_case_indeterminate",
  "agent_name": "echo-image-harmonization",
  "objective": "Run build-phase synthetic workflow",
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "required_skills": ["echo-image-harmonization"],
  "required_output_schema": "schemas/agent_result.schema.json",
  "allowed_write_paths": ["runs/mock_case_indeterminate/agents/echo-image-harmonization/"],
...
```json
{
  ...
  "skills_used": ["echo-image-harmonization"],
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "output_artifacts": [],
  "measurements": [],
  "evidence": {},
