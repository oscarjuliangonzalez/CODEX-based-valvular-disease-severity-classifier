...
Exclude invalid metrics, down-weight context-sensitive metrics, preserve discordance, and return indeterminate when needed. This skill supports research and decision-support development only; it must not be used to claim autonomous clinical diagnosis.

## Trigger Conditions

Use this skill when the active Codex task matches the description above or when an AgentTask lists ar-evidence-integration in required_skills. Do not use it outside AR-focused echocardiography research workflows.
...
8. Populate schemas/agent_result.schema.json for result.json and validate integrated evidence payloads against schemas/evidence_result.schema.json.
...
Return valid JSON matching schemas/agent_result.schema.json for result.json; evidence payloads must also match schemas/evidence_result.schema.json. Prose-only output is invalid.
...
```json
{
  "task_id": "mock_ar-evidence-integration",
  "case_id": "mock_case_indeterminate",
  "agent_name": "ar-evidence-integration",
  "objective": "Run build-phase synthetic workflow",
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "required_skills": ["ar-evidence-integration"],
  "required_output_schema": "schemas/agent_result.schema.json",
  "allowed_write_paths": ["runs/mock_case_indeterminate/agents/ar-evidence-integration/"],
...
```json
{
  ...
  "skills_used": ["ar-evidence-integration"],
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "output_artifacts": [],
  "measurements": [],
  "evidence": {},
