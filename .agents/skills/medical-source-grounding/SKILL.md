...
Map every threshold, formula, limitation, and abstention rule to a local source and provenance entry. This skill supports research and decision-support development only; it must not be used to claim autonomous clinical diagnosis.

## Trigger Conditions

Use this skill when the active Codex task matches the description above or when an AgentTask lists medical-source-grounding in required_skills. Do not use it outside AR-focused echocardiography research workflows.
...
```json
{
  "task_id": "mock_medical-source-grounding",
  "case_id": "mock_case_indeterminate",
  "agent_name": "medical-source-grounding",
  "objective": "Run build-phase synthetic workflow",
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "required_skills": ["medical-source-grounding"],
  "required_output_schema": "schemas/agent_result.schema.json",
  "allowed_write_paths": ["runs/mock_case_indeterminate/agents/medical-source-grounding/"],
...
```json
{
  ...
  "skills_used": ["medical-source-grounding"],
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "output_artifacts": [],
  "measurements": [],
  "evidence": {},
