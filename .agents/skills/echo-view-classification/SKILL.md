...
Return ranked modality/view candidates, evidence, confidence, and abstention status. This skill supports research and decision-support development only; it must not be used to claim autonomous clinical diagnosis.

## Trigger Conditions

Use this skill when the active Codex task matches the description above or when an AgentTask lists echo-view-classification in required_skills. Do not use it outside AR-focused echocardiography research workflows.
...
```json
{
  "task_id": "mock_echo-view-classification",
  "case_id": "mock_case_indeterminate",
  "agent_name": "echo-view-classification",
  "objective": "Run build-phase synthetic workflow",
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "required_skills": ["echo-view-classification"],
  "required_output_schema": "schemas/agent_result.schema.json",
  "allowed_write_paths": ["runs/mock_case_indeterminate/agents/echo-view-classification/"],
...
```json
{
  ...
  "skills_used": ["echo-view-classification"],
  "input_artifacts": ["examples/mock_case/case_manifest.json"],
  "output_artifacts": [],
  "measurements": [],
  "evidence": {},
